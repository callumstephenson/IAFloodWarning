...
def test_stations_by_distance():
    ...
    '''
    assert stations_distance[0][0].name == 'Hemingford'
    assert stations_distance[-1][0].name == 'A6 Bridge'
    assert stations_distance[0][1] == 20.089233515210974
...
def test_rivers_by_station_number():
    ...
    assert len(rivers_by_station_number(stations, 1)) == 2

    river_list = rivers_by_station_number(stations, 2)

    assert len(river_list) == 3

    assert "River 3" not in [item[1] for item in river_list]